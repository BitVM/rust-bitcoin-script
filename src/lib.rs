    ...
    quote!(
        pub mod pushable {
            ...
            impl Chunker {
                ...
                fn highest_chunkable_builder(
                    &mut self,
                ) -> Result<(Box<Builder>, usize), ChunkerError> {
                ...
                pub fn find_chunks(mut self) -> Result<(Vec<usize>, Builder), ChunkerError> {
                    while self.size
                        > self.chunks.last().unwrap_or(&0_usize) + self.target_chunk_size
                    {
